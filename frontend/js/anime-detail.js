...
async function loadAnimeDetail() {
    ...
    try {
        ...
        // Cek history untuk resume timestamp
        ...
        if (anime.episodes && anime.episodes.length > 0) {
            episodeContainer.innerHTML = anime.episodes.map(ep => {
                ...
                return `
                    <div class="episode-button ${isResume ? 'last-watched' : ''}" onclick="playEpisode('${anime.id}', ${ep.number}, '${ep.videoUrl}', '${ep.title.replace(/'/g, "\\'")}', ${resumeTimestamp})">
                        <div>
...
function playEpisode(animeId, episodeNum, videoUrl, episodeTitle, resumeTimestamp = 0) {
    window.location.href = `/watch.html?id=${animeId}&ep=${episodeNum}&video=${videoUrl}&title=${encodeURIComponent(episodeTitle)}&resume=${resumeTimestamp}`;
}
