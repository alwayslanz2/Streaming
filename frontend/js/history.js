...
async function loadHistory() {
    ...
    try {
        ...
        if (todayItems.length > 0) {
            html += '<div class="history-section-title">Hari ini</div>';
            html += todayItems.map(h => renderHistoryItem(h)).join('');
        }
        
        if (yesterdayItems.length > 0) {
            html += '<div class="history-section-title">Kemarin</div>';
            html += yesterdayItems.map(h => renderHistoryItem(h)).join('');
        }
        
        if (olderItems.length > 0) {
            html += '<div class="history-section-title">Sebelumnya</div>';
            html += olderItems.map(h => renderHistoryItem(h)).join('');
        }
...
function renderHistoryItem(h) {
    ...
    // Gunakan cover dari history jika ada, atau placeholder
    const coverUrl = h.cover || 'https://placehold.co/60x85/1a1a1a/888?text=No+Cover';
    
    if (selectedMode === 'multi') {
        return `
            <div class="history-item-selectable ${isSelected ? 'selected' : ''}" data-id="${h._id}" onclick="event.stopPropagation(); toggleSelect('${h._id}')">
        ...
        return `
            <div class="history-item-selectable" onclick="goToAnimeDetail('${h.animeId}', ${h.episode || 1})">
                <img class="history-cover" src="${coverUrl}" onerror="this.src='https://placehold.co/60x85/1a1a1a/888?text=No+Cover'">
                <div class="history-info">
                    <div class="history-title">${h.animeTitle}</div>
                    <div class="history-episode">Episode ${h.episode || 1}</div>
...
function toggleSelect(id) {
    ...
    if (deleteBar) {
        deleteBar.style.display = selectedItems.size > 0 ? 'block' : 'none';
    }
}
...
function goToAnimeDetail(animeId, episode) {
    window.location.href = `/anime-detail.html?id=${animeId}&resumeEp=${episode}`;
}
